...
Chalo ab kuch code examples dekhte hain!''',
    ...
    'challenges': [
      ...
      {
        'question': 'Ek program likho jo user se positive numbers leta rahe (0 se ruk jaye) aur har number ka square nikaalte rahe. Walrus operator use karo while loop mein. End mein "stop" message do.',
        'hint': 'while (n := int(input())) > 0: mein condition use karo. Har baar (sq := n * n) calculate karo aur print karo.',
        'solution': "print(\"Enter positive numbers (0 to stop):\")\nwhile (n := int(input())) > 0:\n    sq = n * n\n    print(f\"Square of {n} is {sq}\")\nprint(\"Program stopped.\")",
        'difficulty': 'medium',
      },
      {
        'question': 'Ek program likho jo file "numbers.txt" se numbers padhe aur unka sum nikaale. Use walrus operator while loop mein file read karte waqt. f.readline() use karo. Jab empty line aaye toh loop break karo.',
        'hint': 'with open() as f: while (line := f.readline()): mein check karo. line.strip() empty ho toh break. Otherwise int mein convert karke sum mein add karo.',
