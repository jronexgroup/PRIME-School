...
Code reuse ka fayda - aapne Employee ka show method nahi likha Programmer mein, lekin woh kaam kar raha hai. Kam code, zyada kaam - inheritance ka yahi maza hai.''',
    ...
    'challenges': [
      ...
      {
        'question': 'Ek multi-level inheritance hierarchy banao: Employee (base) mein name aur salary ho, Developer(Employee) mein technology_stack attribute add ho aur show_details() override kare, TechLead(Developer) mein team_size attribute add ho aur show_details() override kare. Saare levels ke objects bana kar show_details() call karo.',
        'hint': 'class Employee: __init__(self, name, salary). class Developer(Employee): __init__ mein super().__init__(name, salary) aur self.tech = tech. show_details override karo. class TechLead(Developer): similar chain.',
        'solution': "class Employee:\n    def __init__(self, name, salary):\n        self.name = name\n        self.salary = salary\n    def show_details(self):\n        print(f\"Employee: {self.name}, Salary: {self.salary}\")\n\nclass Developer(Employee):\n    def __init__(self, name, salary, tech):\n        super().__init__(name, salary)\n        self.tech = tech\n    def show_details(self):\n        print(f\"Developer: {self.name}, Salary: {self.salary}, Tech: {self.tech}\")\n\nclass TechLead(Developer):\n    def __init__(self, name, salary, tech, team_size):\n        super().__init__(name, salary, tech)\n        self.team_size = team_size\n    def show_details(self):\n        print(f\"TechLead: {self.name}, Salary: {self.salary}, Tech: {self.tech}, Team: {self.team_size}\")\n\ne = Employee(\"Alice\", 50000)\ne.show_details()\nd = Developer(\"Bob\", 60000, \"Python\")\nd.show_details()\nt = TechLead(\"Charlie\", 80000, \"Python\", 5)\nt.show_details()",
        'difficulty': 'hard',
      },
    ],
    'importantSyntax': [
...
Dono ka apna use case hai. Class methods tab use karo jab aapko class level par kuch change karna ho.''',
    ...
    'challenges': [
      ...
      {
        'question': 'Ek multiple inheritance ka example banao: Student class mein name aur roll_no ho, Sports class mein sport_name aur achievement ho, StudentAthlete(Student, Sports) class dono se inherit kare aur display() method mein saari details print kare. MRO ke hisaab se kaunsa method pehle call hoga?',
        'hint': 'class Student: __init__(self, name, roll). class Sports: __init__(self, sport, achievement). class StudentAthlete(Student, Sports): __init__ mein dono parents ke constructors call karo. display() mein saari properties do.',
        'solution': "class Student:\n    def __init__(self, name, roll_no):\n        self.name = name\n        self.roll_no = roll_no\n\nclass Sports:\n    def __init__(self, sport_name, achievement):\n        self.sport_name = sport_name\n        self.achievement = achievement\n\nclass StudentAthlete(Student, Sports):\n    def __init__(self, name, roll_no, sport_name, achievement):\n        Student.__init__(self, name, roll_no)\n        Sports.__init__(self, sport_name, achievement)\n    def display(self):\n        print(f\"Name: {self.name}, Roll: {self.roll_no}\")\n        print(f\"Sport: {self.sport_name}, Achievement: {self.achievement}\")\n\nsa = StudentAthlete(\"Rahul\", 101, \"Cricket\", \"School Captain\")\nsa.display()\nprint(\"MRO:\", StudentAthlete.__mro__)",
        'difficulty': 'medium',
      },
      {
        'question': 'Ek Employee class banao jisme class attribute total_employees = 0 ho aur @classmethod update_count() ho jo total_employees increment kare. Phir Manager(Employee) aur Executive(Manager) banao. Har subclass ke constructor mein super().__init__() ke saath update_count() call karo. Total employees count check karo.',
        'hint': 'class Employee: total_employees = 0. @classmethod def update_count(cls): cls.total_employees += 1. __init__ mein self.update_count(). Manager aur Executive mein super().__init__() call karo.',
        'solution': "class Employee:\n    total_employees = 0\n\n    def __init__(self, name):\n        self.name = name\n        self.update_count()\n\n    @classmethod\n    def update_count(cls):\n        cls.total_employees += 1\n\nclass Manager(Employee):\n    def __init__(self, name, dept):\n        self.dept = dept\n        super().__init__(name)\n\nclass Executive(Manager):\n    def __init__(self, name, dept, level):\n        self.level = level\n        super().__init__(name, dept)\n\ne1 = Employee(\"Alice\")\ne2 = Manager(\"Bob\", \"IT\")\ne3 = Executive(\"Charlie\", \"Sales\", \"Senior\")\nprint(f\"Total employees: {Employee.total_employees}\")\nprint(f\"Employee name: {e1.name}\")\nprint(f\"Manager name: {e2.name}, dept: {e2.dept}\")",
        'difficulty': 'hard',
      },
    ],
    'importantSyntax': [
